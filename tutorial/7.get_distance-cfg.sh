...
#library path add by yangke to fix "ar: `u' modifier ignored since `D' is the default (see `U')"
...
$SUBJECT/testXPath --xptr -i $SUBJECT/test/docs/chapters -f $SUBJECT/test/xptr/chaptersrange
...
$AFLGO/scripts/genDistance.sh $SUBJECT $TMP_DIR testXPath
