...
if [ "$1" != "-" ] ; then
	...
	[ ! -d mjs ] && git clone https://github.com/cesanta/mjs && cd mjs && git checkout 861508b && cd ..

	#setup targets
	if [ "`sed -n 6500p  mjs/mjs.c`" != "if(dst - orig_dst<0){" ] ;then
		sed '6500 iif(dst - orig_dst<0){\nabort();\n}' -i mjs/mjs.c
	fi
	echo -e "mjs.c:6501" >$TMP_DIR/BBtargets.txt
	...
	#sed "22 cCFLAGS += $CFLAGS" -i mjs/mjs/Makefile
	#2nd compile
	#cd mjs/mjs && make clean all && cd -
...
if [[ $AFLGO == *good ]];then
	...
	#/usr/bin/time -a -o $TIME_RECORD_FILE $AFLGO/afl-fuzz -m 100M -S ${TARGET}_$((i))_result -z exp -c $TIME -i $DIR_IN -o $DIR_OUT -E $TMP_DIR  $SUBJECT/mjs/mjs/$TARGET @@
        $AFLGO/afl-fuzz -m 100M -S ${TARGET}_$((i))_result -z exp -c $TIME -i $DIR_IN -o $DIR_OUT -E $TMP_DIR  $SUBJECT/mjs/mjs/$TARGET @@
	if [ "$?" != 0 ];then
		exit
	fi
elif [[ $AFLGO == *origin ]];then
