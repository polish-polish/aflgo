  ...
  if not is_cg :
    ...
    if args.cg_distance is None:
      ...
      with open(args.cg_callsites, 'r') as f:
        for l in f.readlines():
          s = l.strip().split(",")
          if len(s)==1: print("WARNING: line: "+l+" in BBcalls.txt miss callee name. TODO: MODIFY llvm_mode/afl-llvm-pass.so.cc TO FIX THIS CASE") #TODO FIX THIS CASE generated by instrumentation code in aflgo/llvm_mode/afl-llvm-pass.so.cc
          if len(s)>1 and len(find_nodes(s[0])) > 0:
