...
if __name__ == '__main__':
  ...
  parser.add_argument ('-N', '--name', type=str, required=True, help="number of results")
  ...
  for i in range(times):
      #path=args.dir+'/jdmarker_'+str(i+1)+'_result/crashes'
      path=args.dir+'/'+args.name+'_'+str(i+1)+'_result/crashes'
      print path
      temp=cnt
      for fname in os.listdir(path):
          if 'id' in fname:
              ...
              if findit:
                 cnt+=1
      if temp==cnt:
           print "Cannot find any results in result:"+path,
  statistics=[]
